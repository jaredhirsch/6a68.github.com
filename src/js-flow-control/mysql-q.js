...
// hacked to include Q for polemical purposes by 6a68
const q = require('q');
...
exports.userKnown = function(uid, cb) {
  client.query(
    ...
    function(err, rows) {
      ...
      var hasPass = (known && typeof rows[0].passwd === 'string' && rows[0].passwd.length > 0);
...
exports.emailType = function(email, cb) {
  client.query(
    "SELECT type as lastUsedAs FROM email WHERE address = ?", [ email ],
    function(err, rows) {
...
exports.updateIDPLastSeen = function(domain, cb) {
  client.query('INSERT INTO idp (domain, lastSeen) VALUES(?,NOW()) ' +
               'ON DUPLICATE KEY UPDATE lastSeen=NOW()', [ domain ],
               function(err) {
                 cb(err);
               }
              );
...
exports.getIDPLastSeen = function(domain, cb) {
  client.query('SELECT UNIX_TIMESTAMP(lastSeen) AS ts FROM idp WHERE domain = ?', [ domain ],
               function(err, rows) {
                 if (err) cb(err);
...
// 4. verification-related APIs. I think this is the UUID appended to the emailed link.
...
// APIs used to complete the email-mediated flows: user creation/email addition/passwd reset
...
// This appears to only be used for testing
...
// Important but not grouped with similar things in the file.
// Sort of a "db utils" API collection.
...
exports.listEmails = function(uid, cb) {
  client.query(
    'SELECT address FROM email WHERE user = ?',
      [ uid ],
      function (err, rows) {
        if (err) cb(err);
        else {
...
exports.emailLastUsedAs = function(email, cb) {
  client.query('SELECT type as lastUsedAs FROM email WHERE address = ?', [email], function(err, rows) {
    if (err) {
      cb(err);
...
exports.updateEmailLastUsedAs = function(email, type, cb) {
  if (typeEnum.indexOf(type) === -1) {
    ...
    client.query('UPDATE email SET type = ? WHERE address = ?', [type, email], cb);
  }
};
...
exports.cancelAccount = function(uid, cb) {
  client.query("DELETE LOW_PRIORITY FROM email WHERE user = ?", [ uid ], function(err) {
    if (err) return cb(err);
    client.query("DELETE LOW_PRIORITY FROM staged WHERE existing_user = ?", [ uid ], function(err) {
      if (err) return cb(err);
